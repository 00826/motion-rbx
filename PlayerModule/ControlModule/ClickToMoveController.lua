...
local function UpdateIgnoreTag(newIgnoreTag)
	...
	CurrentIgnoreList = table.create(1, GetCharacter())
...
local function Pather(endPoint, surfaceNormal, overrideUseDirectPath)
	...
	this.HumanoidOffsetFromPath = Vector3.zero

	this.CurrentWaypointPosition = nil
	this.CurrentWaypointPlaneNormal = Vector3.zero
	this.CurrentWaypointPlaneDistance = 0
	this.CurrentWaypointNeedsJump = false;

	this.CurrentHumanoidPosition = Vector3.zero
	this.CurrentHumanoidVelocity = 0

	this.NextActionMoveDirection = Vector3.zero
	...
	function this:IsCurrentWaypointReached()
		...
		if this.CurrentWaypointPlaneNormal ~= Vector3.zero then
	...
	local newHitPart, newHitPos = Raycast.RayBlacklist(offsetPoint, Vector3.yAxis * -50, getIgnoreList())
...
function OnTap(tapPositions, goToPoint, wasTouchTap)
	...
	if #tapPositions == 1 or goToPoint then
		if camera then
			...
			local hitPart, hitPt, hitNormal = Raycast.RayBlacklist(unitRay.Origin, unitRay.Direction * 1000, getIgnoreList())
			...
			self.humanoidDiedConn = child.Died:Connect(function()
				if ExistingIndicator then
					task.delay(1, function()
						ExistingIndicator.Model:Destroy()
					end)
				end
			end)
		end
...
function ClickToMove:MoveTo(position, showPath, useDirectPath)
	...
	local thisPather = Pather(position, Vector3.yAxis, useDirectPath)
