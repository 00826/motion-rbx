...
function VRNavigation:TryComputePath(startPos, destination)
	...
	while not newPath and numAttempts < 5 do
		...
		if newPath and newPath.Status == Enum.PathStatus.FailFinishNotEmpty then
			destination = destination + Vector3.yAxis
			newPath = nil
		end
	end
...
function VRNavigation:OnHeartbeat(dt)
	...
	if self.moving and self.currentPoints then
		...
		if moveDist < POINT_REACHED_THRESHOLD then
			...
			local ignoreTable = table.create(1, LocalPlayer.Character)
			local obstructPart, obstructPoint, obstructNormal = Raycast.RayBlacklist(currentPosition - Vector3.yAxis, moveDir * 3, ignoreTable)

			if obstructPart then
				local heightOffset = Vector3.new(0, 100, 0)
				local jumpCheckPart, jumpCheckPoint, jumpCheckNormal = Raycast.RayBlacklist(obstructPoint + moveDir * 0.5 + heightOffset, -heightOffset, ignoreTable)
