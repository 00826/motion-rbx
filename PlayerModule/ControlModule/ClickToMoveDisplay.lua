...
local function CreateWaypointTemplates()
	...
	TrailDotTemplate.Size = Vector3.one
...
local function placePathWaypoint(waypointModel, position)
	local hitPart, hitPoint, hitNormal = Raycast.RayBlacklist(position + Vector3.new(0, 2.5, 0), Vector3.yAxis * -10, table.create(1, LocalPlayer.Character))
	if hitPart then
