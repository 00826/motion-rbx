local Raycast = require(script.Parent.Parent.Parent.Raycast)
...
local blacklist = {} do
	...
	local function refreshIgnoreList()
		...
		for _, Thing in Settings.CameraBlacklist do
			table.insert(blacklist, Thing)
		end
	end

	for _, Thing in Settings.CameraBlacklist do
		Thing.ChildAdded:Connect(refreshIgnoreList)
	end
...
local function getCollisionPoint(origin, dir)
	...
	repeat
		...
		local hitPart, hitPoint = Raycast.RayBlacklist(origin, dir, blacklist)
...
local function queryPoint(origin, unitDir, dist, lastPos)
	...
	repeat
		...
		local entryPart, entryPos = Raycast.RayBlacklist(movingOrigin, target - movingOrigin, blacklist)

		if entryPart then
			if canOcclude(entryPart) then
				local wl = {entryPart}
				--local exitPart = workspace:FindPartOnRayWithWhitelist(ray(target, entryPos - target), wl, true)
				local exitPart = Raycast.RayWhitelist(target, entryPos - target, wl)
				...
				if exitPart then
					...
					if lastPos then
						...
						promote = Raycast.RayWhitelist(lastPos, target - lastPos, wl) or Raycast.RayWhitelist(target, lastPos - target, wl)
					end
