local Raycast = require(script.Parent.Parent.Raycast)
...
local SMART_CIRCLE_INCREMENT = pi2 / SMART_CIRCLE_CASTS
...
function Invisicam:CornerBehavior(castPoints)
	...
	local centerPoint = cframe.Position
...
function Invisicam:CircleBehavior(castPoints)
	...
	for i = 0, CIRCLE_CASTS - 1 do
		local angle = (pi2 / CIRCLE_CASTS) * i
...
function Invisicam:CharacterOutlineBehavior(castPoints)
	...
	table.insert(castPoints, self.torsoPart.CFrame.Position)
	table.insert(castPoints, self.torsoPart.CFrame.Position + torsoUp)
	table.insert(castPoints, self.torsoPart.CFrame.Position - torsoUp)
	table.insert(castPoints, self.torsoPart.CFrame.Position + torsoRight)
	table.insert(castPoints, self.torsoPart.CFrame.Position - torsoRight)
	if self.headPart then
		table.insert(castPoints, self.headPart.CFrame.Position)
	end

	local cframe = CFrame.new(Vector3.zero,Vector3.new(self.camera.CFrame.LookVector.X,0,self.camera.CFrame.LookVector.Z))
	...
	if self.headPart then
		table.insert(partsWhitelist, self.headPart)
	end

	for i = 1, CHAR_OUTLINE_CASTS do
		local angle = (pi2 * i / CHAR_OUTLINE_CASTS)
		local offset = cframe * (3 * Vector3.new(math.cos(angle), math.sin(angle), 0))

		offset = Vector3.new(offset.X, math.max(offset.Y, -2.25), offset.Z)
		local hit, hitPoint = Raycast.RayWhitelist(centerPoint + offset, -3 * offset, partsWhitelist)
...
function Invisicam:SmartCircleBehavior(castPoints)
	...
	for i = 1, SMART_CIRCLE_CASTS do
		...
		local hit, hp, hitNormal = Raycast.RayBlacklist(torsoPoint, circlePoint - torsoPoint, table.create(1, self.char))
		...
		if hit then
			...
			if ( v0.Unit:Dot(-v1) < v0.Unit:Dot(vprime)) then
				...
				if castPoint.Magnitude > 0 then
					local hit, hitPoint, hitNormal = Raycast.RayBlacklist(hprime, castPoint - hprime, table.create(1, self.char))
			...
			local hit, hitPoint, hitNormal = Raycast.RayBlacklist(torsoPoint, (castPoint - torsoPoint), table.create(1, self.char))
...
function Invisicam:Update(dt, desiredCameraCFrame, desiredCameraFocus)
	...
	if USE_STACKING_TRANSPARENCY then
		...
		if (hitPartCount > 0) then
			...
			perPartTransparencyHeadTorsoHits = ((0.5 * TARGET_TRANSPARENCY) + (0.5 * TARGET_TRANSPARENCY / hitPartCount)) ^ (1 / hitPartCount)
			perPartTransparencyOtherHits = ((0.5 * TARGET_TRANSPARENCY_PERIPHERAL) + (0.5 * TARGET_TRANSPARENCY_PERIPHERAL / hitPartCount)) ^ (1 / hitPartCount)
		end
	end
